...
namespace QRCodeCore
{
    internal sealed class QRCodeGenerator
    {
        ...
        private readonly List<AlignmentPattern> alignmentPatternTable = new List<AlignmentPattern>();
        private readonly List<ECCInfo> capacityECCTable = new List<ECCInfo>();
        private readonly List<VersionInfo> capacityTable = new List<VersionInfo>();
        private readonly List<Antilog> galoisField = new List<Antilog>();
        private readonly Dictionary<char, int> alphanumEncDict = new Dictionary<char, int>();
        ...
        public QRCodeMatrix CreateQRCode(string plainText, EccLevel eccLevel, bool utf8BOM = false)
        {
            ...
            for (var i = 0; i < eccInfo.BlocksInGroup1; i++)
            {
                ...
                codeWordWithECC.Add(new CodewordBlock(1, i + 1, bitStr, bitBlockList, eccWordList, bitBlockListDec, eccWordListDec));
            }
            ...
            for (var i = 0; i < eccInfo.BlocksInGroup2; i++)
            {
                ...
                var eccWordList = this.CalculateECCWords(bitStr, eccInfo);
                var eccWordListDec = this.BinaryStringListToDecList(eccWordList);
                codeWordWithECC.Add(new CodewordBlock(2, i + 1, bitStr, bitBlockList, eccWordList, bitBlockListDec, eccWordListDec));
            }
            ...
            var qr = new QRCodeMatrix(version);
        ...
        {
            public static void AddQuietZone(ref QRCodeMatrix qrCode)
            {
            ...
            public static int MaskCode(ref QRCodeMatrix qrCode, int version, ref List<Rectangle> blockedModules, EccLevel eccLevel)
            {
                ...
                foreach (var pattern in typeof(MaskPattern).DeclaredMethods())
                {
                    ...
                    {
                        var qrTemp = new QRCodeMatrix(version);
            ...
            public static void PlaceDataWords(ref QRCodeMatrix qrCode, string data, ref List<Rectangle> blockedModules)
            {
                var size = qrCode.ModuleMatrix.Count;
                var up = true;
            ...
            public static void PlaceDarkModule(ref QRCodeMatrix qrCode, int version, ref List<Rectangle> blockedModules)
            {
                qrCode.ModuleMatrix[4 * version + 9][8] = true;
                blockedModules.Add(new Rectangle(8, 4 * version + 9, 1, 1));
            }

            public static void PlaceFinderPatterns(ref QRCodeMatrix qrCode, ref List<Rectangle> blockedModules)
            {
                var size = qrCode.ModuleMatrix.Count;
            ...
            public static void PlaceAlignmentPatterns(ref QRCodeMatrix qrCode, List<Point> alignmentPatternLocations, ref List<Rectangle> blockedModules)
            {
            ...
            public static void PlaceTimingPatterns(ref QRCodeMatrix qrCode, ref List<Rectangle> blockedModules)
            {
                var size = qrCode.ModuleMatrix.Count;
        ...
        private int GetDataLength(EncodingMode encoding, string plainText, string codedText) => this.IsUtf8(encoding, plainText) ? (codedText.Length / 8) : plainText.Length;

        private bool IsUtf8(EncodingMode encoding, string plainText) => (encoding == EncodingMode.Byte && !this.IsValidISO(plainText));
        ...
        private Polynom XORPolynoms(Polynom messagePolynom, Polynom resPolynom)
        {
            ...
            {
                var polItemRes = new PolynomItem(longPoly.PolyItems[i].Coefficient ^ (shortPoly.PolyItems.Count > i ? shortPoly.PolyItems[i].Coefficient : 0), messagePolynom.PolyItems[0].Exponent - i);
        ...
        private int GetIntValFromAlphaExp(int exp) => this.galoisField.Where(alog => alog.ExponentAlpha == exp).Select(alog => alog.IntegerValue).First();

        private int GetAlphaExpFromIntVal(int intVal) => this.galoisField.Where(alog => alog.IntegerValue == intVal).Select(alog => alog.ExponentAlpha).First();

        private static int ShrinkAlphaExp(int alphaExp) => (int)((alphaExp % 256) + Math.Floor((double)(alphaExp / 256)));
        ...
        private void CreateCapacityTable()
        {
            ...
            {
                this.capacityTable.Add(new VersionInfo((i + 16) / 16,
